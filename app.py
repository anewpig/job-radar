...
_resources_module = importlib.import_module("job_spy_tw.ui.resources")
if not hasattr(_resources_module, "get_salary_estimator"):
    _resources_module = importlib.reload(_resources_module)

from job_spy_tw.models import (
    MarketSnapshot,
)
from job_spy_tw.logging_utils import configure_logging
from job_spy_tw.security import can_access_backend_console
...
from job_spy_tw.ui.loading_shell import (
...
from job_spy_tw.salary_prediction import enrich_jobs_with_salary_estimates

get_salary_estimator = _resources_module.get_salary_estimator
...
def main() -> None:
    ...
    st.session_state._app_render_nonce = uuid4().hex
    boot_loader_placeholder = st.empty()
    should_render_boot_loader = should_show_boot_loader(st.session_state)
    boot_loader_completed = False
    if should_render_boot_loader:
        with boot_loader_placeholder.container():
            render_boot_loading_overlay()
    ...
    try:
        ...
        configure_logging(service_name="app", data_dir=settings.data_dir)
        ...
        current_user_role = getattr(user_state, "current_user_role", "user")
        saved_searches = user_state.saved_searches
        notification_preferences = user_state.notification_preferences
        show_backend_console = bool(settings.show_backend_console) and can_access_backend_console(
            user_role=current_user_role,
            allowed_roles=settings.backend_console_allowed_roles,
        )
        ...
        if snapshot is not None:
            salary_estimator = get_salary_estimator(
                enabled=settings.salary_prediction_enabled,
                model_path=str(settings.salary_prediction_model_path or ""),
            )
            enrich_jobs_with_salary_estimates(snapshot.jobs, salary_estimator)
        ...
        page_context = build_page_context(
            ...
            backend_console_allowed=show_backend_console,
        ...
        page_content_placeholder = st.empty()
        with page_content_placeholder.container():
        ...
        if st.session_state.get("crawl_phase") in {"awaiting_snapshot", "finalizing"}:
        ...
        if should_render_boot_loader:
            dismiss_boot_loading_overlay()
            if boot_loader_completed:
                st.session_state._boot_loader_complete = True
